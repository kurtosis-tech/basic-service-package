...
def make_config_list(services_dict):
    ...
    for service_name in services_dict:
        ...
        service_locator = service.ip_address + ":" + str(service.ports["frontend"].number)
        service_config_string = '{"name": "' + service_name + '", "uri": "' + service_locator + '"}'
...
def run(plan, service_a_count=1, service_b_count=1, service_c_count=1):
    ...
    config_artifact_b = plan.render_templates(
        config={
            "service-config.json": struct(
                template=config_template_b,
                data=template_data_config_b
            )
        }
    )
    ...
    config_artifact_c = plan.render_templates(
        config={
            "service-config.json": struct(
                template=config_template_c,
                data=template_data_config_c
            )
        }
    )
